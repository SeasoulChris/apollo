...
class GenerateSmallRecords(BasePipeline):
    ...
    def run(self, records_rdd, whitelist_dirs_rdd, origin_prefix, target_prefix):
        ...
        tasks = (
            ...
            .groupByKey()
            .collect())
        glog.info('Get {} tasks'.format(len(tasks)))

        sc = self.get_spark_context()
        for task in tasks:
            glog.info('Processing {} records to {}'.format(len(task[1]), task[0]))
            tasks_rdd = sc.parallelize([(task[0], record) for record in task[1]])
            GenerateSmallRecords.process_tasks(tasks_rdd)

    @staticmethod
    def process_tasks(tasks_rdd):
        finished_tasks = (
            # (target_dir, record)
            tasks_rdd
            # -> (target_dir, msg)
            ...
            # -> (target_file, msg)
            .map(GenerateSmallRecords.shard_msg_to_file)
            # -> (target_file, msgs)
            .groupByKey()
            ...
            # Trigger actions.
            .count())
        glog.info('Finished {} tasks'.format(finished_tasks))

    @staticmethod
    def shard_msg_to_file(input):
        """Shard a message to a target file in given dir."""
        target_dir, msg = input
        dt = time_utils.msg_time_to_datetime(msg.timestamp)
        target_file = os.path.join(target_dir, dt.strftime(GenerateSmallRecords.RECORD_FORMAT))

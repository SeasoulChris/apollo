...
def featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, classifierType,
                    modelName, computeBEAT=False, perTrain=0.90):
    '''
...
def featureAndTrainRegression(dirName, mtWin, mtStep, stWin, stStep,
                              modelType, modelName, computeBEAT=False):
    '''
    This function is used as a wrapper to segment-based audio feature extraction and classifier training.
    ARGUMENTS:
    ...
    for c in CSVs:                                                            # for each CSV
        ...
        # read filenames, map to "fileNames" and append respective values in the
        # regressionLabels
        curRegressionLabels = []
        featuresTemp = []
        ...
        with open(c, 'rb') as csvfile:
            ...
            for row in CSVreader:
                # if the current row contains two fields (filename, target value)
                if len(row) == 2:
...
#        modelParams = numpy.array([1, 3, 5, 7, 9, 11, 13, 15]);
    ...
    for iRegression, r in enumerate(regressionNames):
        ...
        bestParam, error, berror = evaluateRegression(
            featuresFinal[iRegression], regressionLabels[iRegression], 100, modelType, modelParams)
        errors.append(error)
...
def evaluateClassifier(features, ClassNames, nExp, ClassifierName,
                       Params, parameterMode, perTrain=0.90):
    '''
    ARGUMENTS:
        features:     a list ([numOfClasses x 1]) whose elements containt numpy matrices of features.
                each matrix features[i] of class i is [numOfSamples x numOfDimensions]
    ...
    for i in range(len(PrecisionClassesAll)):
        ...
        for c in range(len(PrecisionClassesAll[i])):
            print "\t{0:.1f}\t{1:.1f}\t{2:.1f}".format(
                ...
                100.0 * RecallClassesAll[i][c],
                100.0 * F1ClassesAll[i][c]),
...
def trainSpeakerModelsScript():
    ...
    listOfDirs = [
        os.path.join(
            dirName,
            name) for name in os.listdir(dirName) if os.path.isdir(
            os.path.join(
                dirName,
                name))]
    featureAndTrain(

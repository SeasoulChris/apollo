...
def evaluateSpeakerDiarization(flags, flagsGT):
    ...
    for i in range(minLength):
        cMatrix[int(numpy.nonzero(uFlags == flags[i])[0]),
                int(numpy.nonzero(uFlagsGT == flagsGT[i])[0])] += 1.0
...
def trainHMM_computeStatistics(features, labels):
    ...
    for i in range(nComps):
        # cov[i,:,:] = numpy.cov(features[:,numpy.nonzero(labels==uLabels[i])[0]])
        # # use this lines if HMM using full gaussian distributions are to be
        # used!
...
def trainHMM_fromFile(wavFile, gtFile, hmmModelName, mtWin, mtStep):
    ...
    [F, _] = aF.mtFeatureExtraction(x, Fs, mtWin * Fs, mtStep * Fs,
                                    round(Fs * 0.050), round(Fs * 0.050))    # feature extraction
...
def trainHMM_fromDir(dirPath, hmmModelName, mtWin, mtStep):
    ...
    for i, f in enumerate(glob.glob(dirPath + os.sep + '*.wav')):               # for each WAV file
        ...
        if not os.path.isfile(
                gtFile):                                          # if current WAV file does not have annotation -> skip
            continue
...
def evaluateSegmentationClassificationDir(dirName, modelName, methodName):
    ...
    for i, f in enumerate(glob.glob(dirName + os.sep + '*.wav')):            # for each WAV file
        ...
        if methodName.lower() in ["svm", "svm_rbf", "knn", "randomforest",
...
def silenceRemoval(x, Fs, stWin, stStep, smoothWindow=0.5, Weight=0.5, plot=False):
    ...
    while i < len(
            MaxIdx):                                         # for each of the detected onset indices
        curCluster = [MaxIdx[i]]
        if i == len(MaxIdx) - 1:
            break
...
def speakerDiarization(fileName, numOfSpeakers, mtSize=2.0,
                       mtStep=0.2, stWin=0.05, LDAdim=35, PLOT=False):
    '''
    ARGUMENTS:
    ...
    for iSpeakers in sRange:
        ...
        silB = []
        for c in range(
                iSpeakers):                                # for each speaker (i.e. for each extracted cluster)
            ...
            if clusterPerCent < 0.020:
                ...
                silBs = []
                for c2 in range(
                        iSpeakers):                        # compute distances from samples of other clusters
                    if c2 != c:
...
def musicThumbnailing(x, Fs, shortTermSize=1.0, shortTermStep=0.5,
                      thumbnailSize=10.0, Limit1=0, Limit2=1):
    '''

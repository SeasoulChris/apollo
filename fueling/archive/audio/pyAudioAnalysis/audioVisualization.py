        ...
        for j in range(0, N1):
            if str1[j] == s2[i]:
                stringRange[i + 1][j + 1] = min(stringRange[i + 1][j] + 1,
                                                stringRange[i][j + 1] + 1, stringRange[i][j])
            else:
                stringRange[i + 1][j + 1] = min(stringRange[i + 1][j] + 1,
                                                stringRange[i][j + 1] + 1, stringRange[i][j] + 1)
...
def textListToColors(names):
    ...
    for i in range(len(names)):
        for j in range(len(names)):
            Dnames[i, j] = 1 - 2.0 * \
                levenshtein(names[i], names[j]) / float(len(names[i] + names[j]))

...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    '''
    CLASS_NAMES = [
        'bus', 'Car', 'construction_vehicle', 'trailer', 'Truck', 'barrier',
        'Cyclist', 'motorcycle', 'Pedestrian', 'traffic_cone'
    ]
...
def eval_class_v3(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for p, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # print(thresholds)
                all_thresholds[m, p, k, :len(thresholds)] = thresholds
                ...
                for i in range(len(thresholds)):
                    ...
                    precision[m, p, k, i] = pr[i, 0] / (pr[i, 0] + pr[i, 1])
                    if compute_aos:
                        aos[m, p, k, i] = pr[i, 3] / (pr[i, 0] + pr[i, 1])
                for i in range(len(thresholds)):
                    precision[m, p, k, i] = np.max(
                        precision[m, p, k, i:], axis=-1)
                    # recall[m, l, k, i] = np.max(recall[m, l, k, :i + 1], axis=-1)
                    if compute_aos:
                        aos[m, p, k, i] = np.max(aos[m, p, k, i:], axis=-1)
...
def get_official_eval_result(gt_annos,
    ...
    """
    overlap_mod = np.array([[0.7, 0.7, 0.7, 0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0.7, 0.7, 0.7, 0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0.7, 0.7, 0.7, 0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5]])
    overlap_easy = np.array([[0.7, 0.7, 0.7, 0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],

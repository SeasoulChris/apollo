...
class NuScenesDataset(Dataset):
    NumPointFeatures = 5  # xyz, timestamp. set 4 to use kitti pretrain
    NameMapping = {
    ...
    def get_sensor_data(self, query):
        ...
        points = np.concatenate(sweep_points_list, axis=0)[:, [0, 1, 2, 3, 4]]
...
def _fill_trainval_infos(nusc,
    ...
    for sample in prog_bar(nusc.sample):
        ...
        assert Path(lidar_path).exists(), (
            "you must download all trainval data, key-frame only dataset \
             performs far worse than sweeps."
        )
        info = {

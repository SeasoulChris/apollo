...
def noise_per_object_v3_(gt_boxes,
    ...
    if all([c == 0 for c in center_noise_std]) and all(
            [c == 0 for c in rotation_perturb]) and not enable_grot:
        return
                ...
                if ih > 0:
                    for k in range(4):
                        for n in range(4):
                            A = lines_boxes[i, k, 0]
                            B = lines_boxes[i, k, 1]
                            C = lines_qboxes[j, n, 0]
                            D = lines_qboxes[j, n, 1]
                            acd = (D[1] - A[1]) * (C[0] - A[0]) > (
                                C[1] - A[1]) * (D[0] - A[0])
                            bcd = (D[1] - B[1]) * (C[0] - B[0]) > (
                                C[1] - B[1]) * (D[0] - B[0])
                    ...
                    if ret[i, j] is False:
                        ...
                        for n in range(4):  # point l in qboxes
                            for k in range(4):  # corner k in boxes
                                vec = boxes[i, k] - boxes[i, (k + 1) % 4]
                                if clockwise:
                                    vec = -vec
                                cross = vec[1] * (
                                    boxes[i, k, 0] - qboxes[j, n, 0])
                                cross -= vec[0] * (
                                    boxes[i, k, 1] - qboxes[j, n, 1])
                                if cross >= 0:
                        ...
                        if box_overlap_qbox is False:
                            ...
                            for n in range(4):  # point l in boxes
                                for k in range(4):  # corner k in qboxes
                                    vec = qboxes[j, k] - qboxes[j, (k + 1) % 4]
                                    if clockwise:
                                        vec = -vec
                                    cross = vec[1] * (
                                        qboxes[j, k, 0] - boxes[i, n, 0])
                                    cross -= vec[0] * (
                                        qboxes[j, k, 1] - boxes[i, n, 1])
                                    if cross >= 0:  #

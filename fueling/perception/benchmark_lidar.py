...
def execute_task(source_dir):
    ...
    logging.info('start to extract run lidar perception benchmark')
    ...
    logging.info('perception executable command is {}'.format(command))
    return_code = os.system(command)
    logging.info("return code for benchmark is {}".format(return_code))

    cp_command = "cp /apollo/{} {}/results/{}".format(date_time, source_dir, date_time)
    return_code = os.system(cp_command)
    logging.info("return code for copy command is {}".format(return_code))

    if return_code != 0:
        logging.error('failed to execute lidar benchmark for task lidar_benchmark')

 ...
 *****************************************************************************/
#include "cybertron/common/log_file_object.h"
#include <assert.h>
#include <fcntl.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <iomanip>
#include <iostream>
#include "cybertron/common/logger_util.h"
...
void LogFileObject::FlushUnlocked() {
  ...
  const int64_t next = static_cast<int64_t>(2000000);  // in usec
      //(FLAGS_logbufsecs * static_cast<int64_t>(1000000));  // in usec
  next_flush_time_ = ::cybertron::CycleClock_Now() + next;
}
...
void LogFileObject::Write(bool force_flush, time_t timestamp,
  ...
  if (static_cast<int>(file_length_ >> 20) >= ::cybertron::MaxLogSize() ||
      ::cybertron::PidHasChanged()) {
  ...
  if (file_ == NULL) {
    ...
    time_pid_stream << 1900 + tm_time.tm_year << std::setw(2)
                    ...
                    << ::cybertron::GetMainThreadPid();
    const std::string& time_pid_string = time_pid_stream.str();

    std::string host_name;
    ::cybertron::GetHostName(&host_name);
    if (base_filename_selected_) {
      ...
      const std::vector<std::string>& log_dirs =
          ::cybertron::GetLoggingDirectories();
  ...
  if (!stop_writing) {
    ...
    if (::cybertron::CycleClock_Now() >= next_flush_time_) {
  ...
  if (force_flush || (bytes_since_flush_ >= 1000000) ||
      (::cybertron::CycleClock_Now() >= next_flush_time_)) {
    FlushUnlocked();
  }
}

...
def compute_h5_and_gradings(target_groups):
    ...
    if len(msgs) < MSG_PER_SEGMENT:
        glog.warn('no enough items in group {} for task {}'.format(group_id, target))
        return (target, None)
    ...
    h5_utils.write_h5(grading_mtx, 
                      target, 
                      '{}_{}_{:05d}'.format(profiling_conf.vehicle_type,
...
def compute_peak(grading_mtx, arg):
    ...
    elem_num, _ = grading_mtx.shape
    return (np.max(abs(grading_mtx[:, arg.peak_feature_name])) / arg.peak_threshold, elem_num)

def compute_usage(grading_mtx, arg):
    """Compute the usage value"""
    profiling_conf = feature_utils.get_config_control_profiling()
    grading_mtx = filter_value(grading_mtx, arg.usage_thold_value, arg.usage_threshold)
    elem_num, _ = grading_mtx.shape
    if elem_num < profiling_conf.min_sample_size:
        return (0.0, 0)
    return (get_std_value([val / profiling_conf.control_command_pct
                           for val in grading_mtx[:, arg.usage_feature_name]]), elem_num)

def compute_beyond(grading_mtx, arg):
    """Compute the usage value"""
    elem_num, _ = grading_mtx.shape
    return (len(np.where(grading_mtx[:, arg.beyond_feature_name] >= arg.beyond_threshold)),
            elem_num)
...
def combine_gradings(grading_x, grading_y):
    ...
    std_combined = ((std_x**2 * (num_x-1) + std_y**2 * (num_y-1)) / (num_x+num_y)) ** 0.5
    grading_x = grading_x._replace(

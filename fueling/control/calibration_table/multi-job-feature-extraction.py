...
class MultiJobFeatureExtraction(BasePipeline):
    def __init__(self):
        """ initialize """
        BasePipeline.__init__(self, 'multi_job_feature_extraction')
    ...
    def run_prod(self):
        ...
        processed_dirs = spark_helper.cache_and_log(
            ...
            .map(lambda (vehicle, path):
                 (vehicle, path.replace(os.path.join(target_prefix,
    ...
    def run(self, todo_task_dirs, vehicle_conf_folder, origin_prefix, target_prefix):
        ...
        msgs_with_conf = spark_helper.cache_and_log(
            ...
            # PairRDD(vehicle,
            #         ((dir_segment, segment_id, paired_chassis_msg_pose_msg), (vehicle_conf, train_conf)))
            .join(conf), 0)

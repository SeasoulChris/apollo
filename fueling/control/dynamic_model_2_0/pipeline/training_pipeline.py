...
class Training(BasePipeline):
    ...
    @staticmethod
    def train(instance, workers, job_id):
        """Run training task"""
        logging.info(F'instance: {instance}, workers: {workers}, job_id: {job_id}')

        # get dataset
        train_dataset = BosDataset(file_list_dir=training_data_path, is_train=True,
                                   param_file=os.path.join(training_data_path, 'param.bin'))
        ...
        # validate loader
        valid_dataset = BosDataset(file_list_dir=validation_data_path, is_train=False,
                                   param_file=os.path.join(training_data_path, 'param.bin'))
        ...
        # ground truth (dx, dy)
        ax.plot(y[:, 0], y[:, 1], 'o', color='blue', label='Ground truth')
        # # training labels
        ax.plot(train_y[:, 0], train_y[:, 1], 'kx', label='Training ground truth')
        # predicted mean value
        ax.plot(mean[:, 0], mean[:, 1], 's', color='r', label='Predicted mean')
        ax.legend(fontsize=12, frameon=False)
        ax.grid(True)
        # save validation figures to folder
        plt.savefig(os.path.join(result_folder, 'validation.png'))

...
def generate_segments(h5_file):
    segments = []
    glog.info('Loading {}'.format(h5_file))
    with h5py.File(h5_file, 'r') as fin:
        segments = [np.array(segment) for segment in fin.values()]
    glog.info('Segments count: {}'.format(len(segments)))
    return segments


def generate_mlp_data(dataset_path, platform_path, model_folder):
    calibration_table_path = os.path.join(platform_path, 'conf/sim_control_lincoln.pb.txt')
    model_weights_path = os.path.join(model_folder, 'weights.h5')
    model_norms_path = os.path.join(model_folder, 'norms.h5')
    segments = generate_segments(dataset_path)

    total_len = 0
    for segment in segments:
        total_len += (segment.shape[0] - 1)
    ...
    with open(calibration_table_path, 'r') as calibration_table_file:
        lines = calibration_table_file.readlines()
        table_length = len(lines) / 5
        calibration_table = np.zeros([table_length, 3])
        for k, line in enumerate(lines):
            if k % 5 != 0 and k % 5 != 4:
                calibration_table[k / 5, k % 5 - 1] = float(line.split(':')[1])
    ...
    with h5py.File(model_norms_path, 'r') as model_norms_file:
        mean_param_norm = np.array(model_norms_file.get('mean'))
        std_param_norm = np.array(model_norms_file.get('std'))
    ...
    for j, segment in enumerate(segments):
        for k in range(1, segment.shape[0]):
            ...
            Y_imu[i, 0] = segment[k, 8] * np.cos(segment[k, 0]) + segment[k, 9] * \
                np.sin(segment[k, 0])
            ...
            acceleration_point_mass = f(velocity_point_mass, lon_cmd * 100.0)
            Y_point_mass[i, 0] = acceleration_point_mass
            Y_point_mass[i, 1] = segment[k - 1, 17] * 8.203 / 16 * velocity_point_mass / 2.8448
            ...
            acceleration_mlp = Y_mlp[i, 0] * std_param_norm[1] + mean_param_norm[1]
            Y_mlp[i, 0] = acceleration_mlp
            angular_velocity_mlp = Y_mlp[i, 1] * std_param_norm[2] + mean_param_norm[2]
...
def generate_lstm_data(dataset_path, platform_path, model_folder):
    calibration_table_path = os.path.join(platform_path, 'conf/sim_control_lincoln.pb.txt')
    model_weights_path = os.path.join(model_folder, 'weights.h5')
    model_norms_path = os.path.join(model_folder, 'norms.h5')
    segments = generate_segments(dataset_path)
    ...
    for j, segment in enumerate(segments):
        for k in range(1, segment.shape[0]):
            ...
            feature[i, 1] = segment[k, 8] * np.cos(segment[k, 0]) + segment[k, 9] * \
                np.sin(segment[k, 0])
            ...
            if i < DIM_LSTM_LENGTH:
                ...
                acceleration_mlp = segment[k - 1, 8] * np.cos(
                    segment[k - 1, 0]) + segment[k - 1, 9] * np.sin(segment[k - 1, 0])
                angular_velocity_mlp = segment[k - 1, 13]
                Y_mlp[i, 0] = segment[k, 8] * np.cos(segment[k, 0]) + segment[k, 9] * \
                    np.sin(segment[k, 0])
...
def plot_model_output(Y_imu, Y_mlp, Y_point_mass, txt, *pdf):
    ...
    txt.write("Acceleration RMSE of learning-based-model: {}, Error Rate: {}".format(
        RMSE_mlp_acceleration, RMSE_mlp_acceleration / RMS_acceleration))
    txt.write("\n")
    ...
    txt.write("Angular speed RMSE of learning-based-model: {}, Error Rate: {}".format(
        RMSE_mlp_angular_speed, RMSE_mlp_angular_speed / RMS_angular_speed))
    txt.write("\n")
...
def plot_first_integral(I_ground_truth, Y_imu, Y_mlp, Y_point_mass, txt, *pdf):
    ...
    for k in range(1, I_ground_truth.shape[0]):
        I_imu[k, :] = I_imu[k - 1, :] + Y_imu[k, :] * 0.01  # by imu sensor
        I_imu[k, 1] = normalize_angle(I_imu[k, 1])
        I_mlp[k, :] = I_mlp[k - 1, :] + Y_mlp[k, :] * 0.01  # by learning-based-model
    ...
    txt.write("Speed RMSE of learning-based-model: {}, Error Rate: {}".format(
        RMSE_mlp_speed, RMSE_mlp_speed / RMS_speed))
    txt.write("\n")
    ...
    txt.write("Heading RMSE of learning-based-model: {}, Error Rate: {}".format(
        RMSE_mlp_heading, RMSE_mlp_heading / RMS_heading))
    txt.write("\n")
...
def plot_Trajectory(T_ground_truth, I_imu, I_mlp, I_point_mass, txt, *pdf):
    ...
    for k in range(1, T_ground_truth.shape[0]):
    ...
    txt.write("Trajectory RMSE of learning-based-model: {}, Error Rate: {}".format(
        RMSE_mlp_trajectory, RMSE_mlp_trajectory / Trajectory_length))
    txt.write("\n")
...
def evaluate(model_info, dataset_path, platform_path):
    if model_info[0] == 'mlp':
        I_ground_truth, Y_imu, Y_point_mass, Y_mlp, T_ground_truth = generate_mlp_data(
            dataset_path, platform_path, model_info[1])
    ...
    evaluation_result_path = os.path.join(platform_path, 'evaluation_result/evaluation_metrics_for_' +
               model_info[0] + '_model.txt')
    with open(evaluation_result_path, 'a') as txt:
        txt.write('\n evaluted on model: {} \n'.format(model_info[1]))
        txt.write('evaluted on record: {} \n'.format(dataset_path))
        plot_model_output(Y_imu, Y_mlp, Y_point_mass, txt)
        I_imu, I_mlp, I_point_mass = plot_first_integral(
            I_ground_truth, Y_imu, Y_mlp, Y_point_mass, txt)
        plot_Trajectory(T_ground_truth, I_imu, I_mlp, I_point_mass, txt)

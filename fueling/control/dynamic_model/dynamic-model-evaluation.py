...
class DynamicModelEvaluation(BasePipeline):
    ...
    def run_test(self):
        ...
        mlp_model_path = os.path.join(
            platform_path, 'dynamic_model_output/h5_model/mlp/*')
        lstm_model_path = os.path.join(
            platform_path, 'dynamic_model_output/h5_model/lstm/*')

        # PairRDD(model_name, folder_path)
        mlp_model_rdd = self.to_rdd(
            glob.glob(mlp_model_path)).keyBy(lambda _: 'mlp')
        # PairRDD(model_name, folder_path)
        lstm_model_rdd = self.to_rdd(
            glob.glob(lstm_model_path)).keyBy(lambda _: 'lstm')
        ...
        self.model_evalution(
            mlp_model_rdd, evaluation_dataset_rdd, platform_path)
        self.model_evalution(
            lstm_model_rdd, evaluation_dataset_rdd, platform_path)

    def run_prod(self):
        platform_path = 'modules/control/learning_based_model/'
        mlp_model_prefix = os.path.join(
            platform_path, 'dynamic_model_output/h5_model/mlp')
        lstm_model_prefix = os.path.join(
            platform_path, 'dynamic_model_output/h5_model/lstm')
        data_predix = os.path.join(
            platform_path, 'hdf5_evaluation', VEHICLE_ID, 'golden_test')

        bos = self.bos()
        # PairRDD('mlp', folder_path)
        mlp_model_rdd = self.to_rdd(bos.list_dirs(
            mlp_model_prefix)).keyBy(lambda _: 'mlp')
        # PairRDD('lstm', folder_path)
        lstm_model_rdd = self.to_rdd(bos.list_dirs(
            lstm_model_prefix)).keyBy(lambda _: 'lstm')

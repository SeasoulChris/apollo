...
class NCLTFilter():
    ...
    def run(self, t, x0, P0, u_fog, u_odo, y_imu, compare):
        """Run test with trained model with filter"""
        N_t = t.shape[0]
        self.x = x0.clone()
        self.P = P0.clone()
    ...
    def propagate(self, u_odo, u_fog, delta_t, compare):
        """Porpagate states of trained model during test run"""
        self.x_prev = self.x
        for i in range(u_odo.shape[0]):
            self.integrate_odo_fog(u_odo[i], u_fog[i], delta_t)
        if self.gp_odo_fog:
            self.x, G_cor = self.gp_odo_fog.correct(self.x, u_odo, u_fog,
                                                    compute_G=(compare == 'filter'))
        else:
    ...
    def compute_jac_update(self, u_odo):
        """Update jacobian of the nonlinear models"""
        J = self.J
        H = self.H
    ...
    def compute_cate(self, t, x, chi, dataset_name):
        """Compute the cumulative absolute trajectory errors"""
    ...
    def compute_mate(self, t, x, chi, dataset_name):
        """Compute the mean absolute trajectory errors"""
        chi_est = torch.zeros(x.shape[0], 4, 4)
        chi_est[:, :3, :3] = SO3.from_rpy(x[:, 3:6]).as_matrix()
        chi_est[:, :3, 3] = x[:, :3]
        chi_est[:, 3, 3] = 1
    ...
    def compute_error(self, t, x, chi, dataset_name):
        """Compute the overall trajectory errors"""
        ...
        error = {'mate translation': mate_translation,
...
class KAISTFilter(NCLTFilter):
    """Desing EKF filter for the data in the KAIST format"""
    ...
    def encoder2speed(self, u_odo, dt):
        """Transfer the encoder signal into speed data"""

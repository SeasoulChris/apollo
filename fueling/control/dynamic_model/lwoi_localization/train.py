...
class GpOdoFog(Parameterized):
    ...
    def correct_cov(self, u_odo_fog, x_cor, compute_G):
        """Correctness covariance of state errors"""
    ...
    def encoder2speed(self, u_odo, delta_t):
        """Transfer the encoder signal to speed"""
...
class GpImu(Parameterized):
    """Gaussian process of IMU signal"""
    ...
    def model(self):
        """IMU Gaussian process model"""
        pyro.module("GpImu", self)
        return self.gp_h.model()

    def set_data(self, u, y):
        """Set data for the IMU Gaussian process model"""
    ...
    def correct_cov(self, u_imu, y_cor):
        "Correctness covarance of the state errors"
...
def preprocessing(args, dataset, gp):
    """Preprocessing of the dataset for training based on the arguments"""
...
def save_gp(args, gp_model, kernel_net):
    ...
    if gp_model.name == 'GpOdoFog':
        name = "gp_odo_fog"
        torch.save(gp_model.gp_f.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "gp_f.p"))
        torch.save(gp_model.gp_f.kernel.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "kernel.p"))
        torch.save(gp_model.gp_f.likelihood.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "likelihood.p"))
        torch.save(kernel_net.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "fnet.p"))
    else:
        name = 'gp_imu'
        torch.save(gp_model.gp_h.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "gp_h.p"))
        torch.save(gp_model.gp_h.kernel.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "kernel.p"))
        torch.save(gp_model.gp_h.likelihood.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "likelihood.p"))
        torch.save(kernel_net.state_dict(),
                   os.path.join(args.path_temp, args.dataset_name, name, "hnet.p"))

    torch.save(gp_model.normalize_factors,
               os.path.join(args.path_temp, args.dataset_name, name, "normalize_factors.p"))
...
def train_gp(args, dataset, gp_class):
    ...
    u, y = (dataset.get_train_data(0, gp_class.name) if args.nclt else
            dataset.get_test_data(1, gp_class.name))
    ...
    if gp_class.name == 'GpOdoFog':
        save_gp(args, gp_instante, fnet)
    else:
        save_gp(args, gp_instante, hnet)

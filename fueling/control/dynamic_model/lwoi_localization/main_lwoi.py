...
def read_data_nclt(args):
    ...
    def set_path_nclt(args, datasetpath, dataset):
        """Set up path for the extracted data"""
        path_odo = os.path.join(datasetpath, "wheels.csv")
        path_fog = os.path.join(datasetpath, "kvh.csv")
        path_imu = os.path.join(datasetpath, "ms25.csv")
        path_gt = os.path.join(args.path_data_base, 'ground_truth',
                               args.dataset_name, "groundtruth_" + dataset + ".csv")
    ...
    def interp_data(x, t, t0):
        """Interpolate the extracted data"""
    ...
    for idx_i, datasetpath_i in enumerate(datasets):
        ...
        path_odo, path_fog, path_imu, path_gt = set_path_nclt(args, datasetpath_i, dataset_i)
...
def read_data_kaist(args):
    ...
    def set_path_kaist(args, datasetpath, dataset):
        """Set up path for the extracted data"""
        path_odo = os.path.join(datasetpath, "encoder.csv")
        path_fog = os.path.join(datasetpath, "fog.csv")
        path_imu = os.path.join(datasetpath, "xsens_imu.csv")
        path_gt = os.path.join(args.path_data_base, 'ground_truth',
                               args.dataset_name, dataset, "global_pose.csv")
        return path_odo, path_fog, path_imu, path_gt
    ...
    for idx_i, datasetpath_i in enumerate(datasets):
        dataset_i = os.path.basename(datasetpath_i)
        print("\nDataset name: " + dataset_i)
        path_odo, path_fog, path_imu, path_gt = set_path_kaist(args, datasetpath_i, dataset_i)
...
def set_gp_imu(args, dataset):
    ...
    hnet_dict = torch.load(os.path.join(path_gp_imu, "hnet.p"))
    lik_dict = torch.load(os.path.join(path_gp_imu, "likelihood.p"))
    kernel_dict = torch.load(os.path.join(path_gp_imu, "kernel.p"))
    gp_dict = torch.load(os.path.join(path_gp_imu, "gp_h.p"))
    ...
    kernel_h_origin = gp.kernels.Matern52(input_dim=args.kernel_dim,
                                          lengthscale=torch.ones(args.kernel_dim))
    kernel_h = gp.kernels.Warping(kernel_h_origin, iwarping_fn=hnet_fn)
    kernel_h.load_state_dict(kernel_dict)
    gp_h = gp.models.VariationalSparseGP(u, u.new_ones(9, u.shape[0]), kernel_h, Xu,
                                         num_data=dataset.num_data, likelihood=lik_h,
...
def set_gp_odo_fog(args, dataset):
    ...
    fnet_dict = torch.load(os.path.join(path_gp_odo_fog, "fnet.p"))
    lik_dict = torch.load(os.path.join(path_gp_odo_fog, "likelihood.p"))
    kernel_dict = torch.load(os.path.join(path_gp_odo_fog, "kernel.p"))
    gp_dict = torch.load(os.path.join(path_gp_odo_fog, "gp_f.p"))
    ...
    kernel_f_origin = gp.kernels.Matern52(input_dim=args.kernel_dim,
                                          lengthscale=torch.ones(args.kernel_dim))
    kernel_f = gp.kernels.Warping(kernel_f_origin, iwarping_fn=fnet_fn)
    kernel_f.load_state_dict(kernel_dict)
    gp_f = gp.models.VariationalSparseGP(u, u.new_ones(6, u.shape[0]), kernel_f, Xu,
                                         num_data=dataset.num_data, likelihood=lik_f,
                                         mean_function=None, whiten=True, jitter=1e-4)
...
def post_tests(args, dataset, filter_original):
    ...
    for i in range(len(dataset.datasets)):
        ...
        x_corrected, P_corrected = filter_corrected.run(t, x0, P0, u_fog, u_odo, y_imu,
                                                        args.compare)
        x_original, P_original = filter_original.run(t, x0, P0, u_fog, u_odo, y_imu,
                                                     args.compare)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GP lwoi')
    parser.add_argument('--nclt', type=bool, default=True)

    # paths
    parser.add_argument('--path_data_base', type=str,
                        default="testdata/control/lwoi_localization/")
    parser.add_argument('--path_data_save', type=str,
                        default="testdata/control/lwoi_localization/data/")
    parser.add_argument('--path_results', type=str,
                        default="testdata/control/lwoi_localization/results/")
    parser.add_argument('--path_temp', type=str,
                        default="testdata/control/lwoi_localization/temp/")

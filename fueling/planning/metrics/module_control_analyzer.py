...
class ControlAnalyzer:
    ...
    def print_latency_statistics(self):
        ...
        print("\n\n")
        print(PrintColors.HEADER + "--- Control Latency (ms) ---" + PrintColors.ENDC)
        analyzer = StatisticalAnalyzer()
        analyzer.print_statistical_results(self.module_latency)

        print(PrintColors.HEADER + "--- station error ---" + PrintColors.ENDC)
        analyzer.print_statistical_results(self.lon_station_error)

        print(PrintColors.HEADER + "--- speed error ---" + PrintColors.ENDC)
        analyzer.print_statistical_results(self.lon_speed_error)

        print(PrintColors.HEADER + "--- lateral error ---" + PrintColors.ENDC)
        analyzer.print_statistical_results(self.lat_lateral_error)

        print(PrintColors.HEADER + "--- heading error ---" + PrintColors.ENDC)
        analyzer.print_statistical_results(self.lat_heading_error)

        print(PrintColors.HEADER + "--- Control Error Code Distribution ---" + PrintColors.ENDC)
        self.error_code_analyzer.print_results()

        print(PrintColors.HEADER + "--- Control Error Msg Distribution ---" + PrintColors.ENDC)

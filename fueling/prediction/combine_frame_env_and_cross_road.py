...
class CombineFrameEnvAndCrossRoad(BasePipeline):
    ...
    def GetObstacleOutput(self, obstacle_history, cross_road_dict_merged):
        ...
        obstacle_output[1].append(obstacle_id)
        key = '{}@{:.3f}'.format(obstacle_id, obstacle_ts)
        obstacle_output[2].append(0)
        if obstacle_type == TARGET_OBSTACLE_TYPE and \
           key in cross_road_dict_merged:
            obstacle_output[2][0] = cross_road_dict_merged[key]
        ...
        for frame_env_filename in frame_env_files:
            ...
            for frame_env in frame_envs.frame_env:
                ...
                if frame_env.ego_history is None:
                    continue
                ego_output = self.GetObstacleOutput(frame_env.ego_history, cross_road_dict_merged)
...
if __name__ == '__main__':
    CombineFrameEnvAndCrossRoad().main()
